...
double death_TC_blood  = koutTC * CLAMP(TC_BLOOD);
double death_TC_spleen = koutTC * CLAMP(TC_SPLEEN);
double death_TC_node   = koutTC * CLAMP(TC_NODE);
double death_TC_lymph  = koutTC * CLAMP(TC_LYMPH);

// T-Cells: Trafficking Fluxes
// Logic: We can only move cells that actually exist
double traf_TC_blood_spleen = kpt * (1 + INJ_Scaler * INJ) * CLAMP(TC_BLOOD);
double traf_TC_spleen_node  = ktn * CLAMP(TC_SPLEEN);
double traf_TC_node_lymph   = knl * CLAMP(TC_NODE);
double traf_TC_lymph_blood  = klp * CLAMP(TC_LYMPH);

// B-Cells: Production & Death
double prod_BC         = kinBC * pow(CLAMP(AF_BC), r);
double death_BC_blood  = koutBC * CLAMP(BC_BLOOD);
double death_BC_spleen = koutBC * CLAMP(BC_SPLEEN);
double death_BC_node   = koutBC * CLAMP(BC_NODE);
double death_BC_lymph  = koutBC * CLAMP(BC_LYMPH);

// B-Cells: Trafficking Fluxes
double traf_BC_blood_spleen = kpt * (1 + INJ_Scaler * INJ) * CLAMP(BC_BLOOD);
double traf_BC_spleen_node  = ktn * CLAMP(BC_SPLEEN);
double traf_BC_node_lymph   = knl * CLAMP(BC_NODE);
double traf_BC_lymph_blood  = klp * CLAMP(BC_LYMPH);